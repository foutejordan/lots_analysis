...
def main_function(df_lots, df_agents, df_buyers, df_suppliers):
    ...
    # Draw edges
    nx.draw_networkx_edges(G, pos, width=[d['weight'] for u, v, d in G.edges(data=True)], edge_color='skyblue')
    ...
    with open('graphes/cities_communities.txt', 'w') as f:
        f.write("Communities:\n")
        for i, community in enumerate(best_partition):
            f.write("Community {}: {}\n".format(i+1, community))

    plt.savefig('graphes/graphe_cities.png')
    plt.show()
